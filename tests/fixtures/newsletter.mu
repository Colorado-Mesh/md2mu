> `c`F000Colorado Mesh Monthly - August & September 2026`f
`a
Hey folks, and welcome to the very first Colorado Mesh Monthly. Since this is our kickoff and we're publishing mid-month, we're reaching back across both August and September so nobody misses the good stuff. Going forward this lands once a month - but for now, settle in: it was a huge two months. We linked the mesh across state lines into Nebraska, kept pushing the Denver metro onto MediumFast, started seriously mapping out regional "scopes," and welcomed a genuinely wonderful flood of new operators. Here's the recap, sorted by protocol so you can jump straight to your corner of the mesh.

>> `c`F8f0General News`f
`a
...
`(Colorado Mesh at the RMHAM Summer Swapfest, Aug 23 (photo: JohnC)`w=80`a=c`:/media/cmesh-blog/img/event_2026-09-17_JohnC_2_web.webp)
`(The Colorado Mesh table at the Swapfest (photo: JohnnyW5KV)`w=80`a=c`:/media/cmesh-blog/img/event_2026-09-17_JohnnyW5KV_1_web.webp)
...
>> `c`F8f0Photo / Video of the Month`f
`a
`(Mt Blue Sky summit`w=80`a=c`:/media/cmesh-blog/img/bluesky_b_square_web.webp)

`cCaption: On the summit of Mt Blue Sky (14,130 ft), M0TH3R logged which repeaters heard - and were heard by - a node at the top of Colorado. Photo: M0TH3R, Aug 31.
`a
...
`(beala's hand-built 900 MHz bandpass filter (#projects-and-builds)`w=80`a=c`:/media/cmesh-blog/img/filter_web.webp)
...
>> `c`F8f0Story of the Month: Meshing the Divide`f
`a
...
>> `c`F8f0Sources & Further Reading`f
`a
